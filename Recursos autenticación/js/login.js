...
// Usar Selector para capturar el formulario de Login.

const loginForm = document.getElementById("loginForm")

// Escuchar el evento submit del formulario y llamar funcion de ingresarUsuario
...
function ingresarUsuario (e) {
    ...
    if (!existeUsuario) { //usuario no existe
        alert ("El usuario no existe, por favor registrate")
        loginForm.reset()
        return 
    }

    if (existeUsuario.password !== password) { // si no concuerda la contraseña
    ...
    localStorage.setItem("logueado",JSON.stringify(usuarioLogueado)) // Guarda el usuarioLogueado en el LocalStorage
